...
export default function FleetContent({ vehicles }) {
  ...
  if (vehicles.length === 0) {
    return (
      <motion.div
        ...
        className="text-center py-12 sm:py-16 md:py-20 bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-xl mx-4 sm:mx-0"
      >
        <p className="text-xl sm:text-2xl text-gray-600">Aucun véhicule disponible pour le moment.</p>
        <p className="text-sm sm:text-base text-gray-500 mt-2">Revenez bientôt ou contactez-nous directement.</p>
        <Link
          href="https://wa.me/237673342789"
          className="inline-flex items-center gap-2 bg-green-600 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-full mt-4 sm:mt-6 hover:bg-green-700 transition text-sm sm:text-base"
        >
  ...
  return (
    <>
          ...
          <button
            ...
            className={`px-4 sm:px-6 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 shadow-md hover:shadow-xl ${
              filter === filterName
                ...
                : 'bg-white/90 backdrop-blur-sm text-gray-700 hover:bg-blue-600 hover:text-white'
            }`}
          >
            {filterName}
          </button>
      ...
      {/* Message de disponibilité - responsive */}
      <div className="mt-10 sm:mt-12 md:mt-16 text-center text-gray-500 text-xs sm:text-sm px-4">
